...
// ===== Controller selection (Arduino-friendly) =====
// Enable exactly ONE controller; default is SH1106 for your current build.
#ifndef PRA32_U_OLED_IS_SH1106
//#define PRA32_U_OLED_IS_SH1106 0      // default: SH1106
#endif
// #define PRA32_U_OLED_IS_SSD1306 1
 #define PRA32_U_OLED_IS_SSD1309 1

#if (defined(PRA32_U_OLED_IS_SH1106) + defined(PRA32_U_OLED_IS_SSD1306) + defined(PRA32_U_OLED_IS_SSD1309)) > 1
  #error "Select only one: PRA32_U_OLED_IS_SH1106 or PRA32_U_OLED_IS_SSD1306 or PRA32_U_OLED_IS_SSD1309"
#endif

#ifndef PRA32_U_OLED_SSD1306_X_OFFSET
#define PRA32_U_OLED_SSD1306_X_OFFSET 0
#endif
#ifndef PRA32_U_OLED_SH1106_X_OFFSET
#define PRA32_U_OLED_SH1106_X_OFFSET 4   // typical 2/4/6/8
#endif
#ifndef PRA32_U_OLED_SSD1309_X_OFFSET
#define PRA32_U_OLED_SSD1309_X_OFFSET 0
#endif

#if defined(PRA32_U_OLED_IS_SH1106)
  #define PRA32_U_OLED_X_OFFSET (PRA32_U_OLED_SH1106_X_OFFSET)
#elif defined(PRA32_U_OLED_IS_SSD1309)
  #define PRA32_U_OLED_X_OFFSET (PRA32_U_OLED_SSD1309_X_OFFSET)
#else /* SSD1306 */
  #define PRA32_U_OLED_X_OFFSET (PRA32_U_OLED_SSD1306_X_OFFSET)
#endif
// ===================================================
...
static INLINE void PRA32_U_ControlPanel_set_draw_position(uint8_t x, uint8_t y) {const uint8_t x_offset = (uint8_t)PRA32_U_OLED_X_OFFSET;
const uint8_t col = (uint8_t)(x * 6 + x_offset);

// Clear hidden left columns on this page once per line to avoid a bright strip (only when offset>0).
if (x == 0 && x_offset > 0) {
  ...
  uint8_t zeros[1 + PRA32_U_OLED_X_OFFSET];
  zeros[0] = 0x40;
  for (uint8_t i = 1; i <= PRA32_U_OLED_X_OFFSET; ++i) zeros[i] = 0x00;
...
// --- Helper: write two zero columns at end of a 21-char line (21*6=126) ---
static INLINE void PRA32_U_ControlPanel_line_tail(uint8_t y) {
  const uint8_t col_end = (uint8_t)(PRA32_U_OLED_X_OFFSET + 126);
...
// --- Optional: SSD1309 init helper (call once in setup when SSD1309 is selected) ---
#ifndef PRA32_U_SSD1309_USE_CHARGEPUMP_STYLE
#define PRA32_U_SSD1309_USE_CHARGEPUMP_STYLE 0   // 0: native 1309 DC-DC, 1: 1306-style charge pump
#endif
static INLINE void PRA32_U_ControlPanel_init_display_SSD1309(void) {
  const uint8_t init_seq[] = {
    ...
    0x40,            // start line
    0xA1,            // seg remap (flip with 0xA0 if mirrored)
    0xC8,            // COM scan dir (flip with 0xC0 if upside-down)
    0xDA, 0x12,      // COM pins
    0x81, 0x80,      // contrast
    0xD9, 0x22,      // precharge
    0xDB, 0x20,      // VCOMH
    0xA4,            // resume
    0xA6,            // normal
#if PRA32_U_SSD1309_USE_CHARGEPUMP_STYLE
    0x8D, 0x14,      // 1306-style charge pump ON
#else
    0xAD, 0x8A,      // 1309 native DC-DC enable
#endif
    0xAF             // display on
  };
  i2c_write_blocking(PRA32_U_OLED_DISPLAY_I2C, PRA32_U_OLED_DISPLAY_I2C_ADDRESS, init_seq, sizeof(init_seq), false);
}
// <<< ВСТАВЬ СРАЗУ ПОСЛЕ init_display_SSD1309 >>>
static inline void PRA32_U_ControlPanel_normal_mode(void) {
  // why: гарантируем Resume/Normal после init
  const uint8_t cmds[] = { 0x00, 0xA4, 0xA6 };
  i2c_write_blocking(
      PRA32_U_OLED_DISPLAY_I2C,
      PRA32_U_OLED_DISPLAY_I2C_ADDRESS,
      cmds, sizeof(cmds),
      false
  );
}